#+TODO: TODO IN_PROGRESS WAITING DONE * emacs大法好 
** 一年之内，学成emacs这套刀法
...
***** insert A to Z   
   A Put your cursor before somewords.
   
   B Call /set-mark-command/ *C-space*.
   
   C Move cursor to end of somewhere you want.
   
   D Call /replace-rectangle/ *C-u*
   
   E Now call /rectangle-number-lines/. It will prompt you to enter arguments
   
   F Type 65 (Letter *A* has Unicode codepoint =65=. *a* is =97=).
   
   G Remove the default *%2d*, type *%c*(the "%c" is for character format)
